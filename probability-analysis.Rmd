...
coin_flip_graph_vec <- function(numTrials, flipNums) {
  ...
  firstCount <- 1
  for (count in counts) {
    numFlips <- append(numFlips, flipNums[firstCount])
    prob <- append(prob, count / numTrials)
    firstCount <- firstCount + 1
  }
  prob_graph <- prob_tibble %>% 
...
multi_face_coin_flip_graph <- function(numOutcomesVec, numTrials, lowerFlipNum, upperFlipNum) {
  prob_tibble <- tibble(numFlips = numeric(), prob = numeric(), numCoinFaces = numeric())
  numFlips <- c()
  prob <- c()
  numCoinFaces <- c()
  for (numOutcomes in numOutcomesVec) {
    ...
    for (count in counts) {
      numFlips <- append(numFlips, firstCount)
      prob <- append(prob, count / numTrials)
      numCoinFaces <- append(numCoinFaces, numOutcomes)
      firstCount <- firstCount + 1
    }
  }
  prob_graph <- prob_tibble %>%
...
multi_face_prob <- function(numFaces, numTrials, numRolls) {
  firstGroup <- matrix(sample(1:numFaces, size = numTrials * numRolls, replace = TRUE), nrow = numTrials, ncol = numRolls)
  firstGroupMeans <- apply(firstGroup, 1, mean)
  secondGroup <- matrix(sample(1:numFaces, size = numTrials * numRolls, replace = TRUE), nrow = numTrials, ncol = numRolls)
  secondGroupMeans <- apply(secondGroup, 1, mean)
  comparisonMeans <- firstGroupMeans == secondGroupMeans
  return(mean(comparisonMeans))
}

multi_face_graph <- function(facesVector, numTrials, lowerFlipNum, upperFlipNum) {
  probTibble <- tibble(numFaces = numeric(), numRolls = numeric(), prob = numeric())
  numFaces <- c()
  numRolls <- c()
  prob <- c()
  for (a in facesVector) {
    for (i in lowerFlipNum:upperFlipNum) {
      numFaces <- append(numFaces, a)
      numRolls <- append(numRolls, i)
      prob <- append(prob, multi_face_prob(a, numTrials, i))
    }
  }
  print(probTibble)
  graph <- probTibble %>% 
    add_row(numFaces, numRolls, prob) %>% 
    ggplot(mapping = aes(numRolls, prob, color = numFaces)) + 
    geom_point() + 
    theme_bw() + 
...
multi_face_graph(c(2, 3, 5, 10), 10000, 1, 100)
coin_flip_graph(8000, 1, 100)
coin_flip_graph_vec(8000, c(1000, 10000, 100000, 1000000))
